...
int main(void) {
    ...
    while(1) {
        ...
        // loops for drawing square (base of pyramid)
        for (float i = -pyramidBaseSize; i < pyramidBaseSize; i += 0.5) {
            for (float j = -pyramidBaseSize; j < pyramidBaseSize; j += 0.5) {
                rotatedPoint(i, pyramidBaseSize, j, '.');
            }
        }
        ...
        for (float t = pyramidBaseSize; t >= pyramidApexY; t -= 0.5) {
            ...
            for (float u = -triangleWidth; u <= triangleWidth; u += 0.5) {
                rotatedPoint(pyramidBaseSize - triangleTilt, t, u, '@');
                rotatedPoint(-pyramidBaseSize + triangleTilt, t, u, '!');
                rotatedPoint(u, t, pyramidBaseSize - triangleTilt, ';');
                rotatedPoint(u, t, -pyramidBaseSize + triangleTilt, '*');
            }
            triangleTilt += 0.20;
        }
